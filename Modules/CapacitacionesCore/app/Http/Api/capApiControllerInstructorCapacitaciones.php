...
use Modules\CapacitacionesCore\App\Models\Escuela\Audience;
...
use Modules\CapacitacionesCore\App\Models\Escuela\Goal;
...
use Modules\CapacitacionesCore\App\Models\Escuela\Lesson;
use Modules\CapacitacionesCore\App\Models\Escuela\Level;
use Modules\CapacitacionesCore\App\Models\Escuela\Requirement;
use Modules\CapacitacionesCore\App\Models\Escuela\Section;
...
class capApiControllerInstructorCapacitaciones extends Controller
{
    ...
    public function capFunctionIndexEstudiantes($id_course)
    {
        ...
        $grupos_array = Grupo::select('id', 'grupo')->get()->sortBy('grupo')->map(fn($grupo) => [
            'id_grupo' => $grupo->id,
            'name_grupo' => $grupo->grupo
        ])->toArray();
    ...
    public function capFunctionStoreEstudiantes(Request $request, $id_course)
    {
        ...
        if ($request->publico === 'grupo') {
            ...
            $grupoNombre = Grupo::where('id', $request->id_grupo)->value('grupo'); // Optimizado con `value()`

            return response()->json([
                'status' => 'success',
                'message' => "Los estudiantes del área $grupoNombre se han agregado exitosamente"
            ]);
        }
